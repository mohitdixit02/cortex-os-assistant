...
import { useThreads, useTasks, ChatThread, Task } from '../../hooks/useApi';
...
export default function History() {
  ...
  const getStatusIcon = (status: string) => {
    switch (status.toLowerCase()) {
      ...
      case 'processing': return <FaSpinner className="spin" color="#3b82f6" />;
      ...
      <div style={{ 
        ...
        >
          ...
          {activeTab === 'conversations' && (
            <motion.div layoutId="tab-underline" style={{ position: 'absolute', bottom: '-11px', left: 0, right: 0, height: '2px', background: 'var(--accent-purple)' }} />
          )}
        </button>
        <button 
            ...
            {threadsLoading ? (
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
                  ...
                  <div>
                    <h3 style={{ fontSize: '18px', fontWeight: '600', marginBottom: '4px' }}>{thread.summary || "New Conversation"}</h3>
              ...
              <>
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: '20px', flex: 1 }}>
                      <div style={{ 
                        ...
                        <FaTasks color="var(--accent-blue)" />
                      </div>
                      <div style={{ flex: 1 }}>
                        <h3 style={{ fontSize: '18px', fontWeight: '600', marginBottom: '4px' }}>{task.task_name}</h3>
                        <p style={{ fontSize: '14px', color: 'var(--text-muted)', marginBottom: '8px', maxWidth: '500px' }}>{task.task_description}</p>
                ...
                <div style={{ display: 'flex', justifyContent: 'center', gap: '10px', marginTop: '20px' }}>
                  <button 
                    ...
                    style={{ padding: '8px 16px', borderRadius: '8px', background: 'rgba(255,255,255,0.05)', color: 'white', opacity: taskPage === 1 ? 0.5 : 1 }}
                  >
                    Previous
                  </button>
                  ...
                  <button 
                    ...
                    onClick={() => setTaskPage(p => p + 1)}
                    style={{ padding: '8px 16px', borderRadius: '8px', background: 'rgba(255,255,255,0.05)', color: 'white', opacity: (tasks && tasks.length < 20) ? 0.5 : 1 }}
                  >
                    Next
                  </button>
                </div>
