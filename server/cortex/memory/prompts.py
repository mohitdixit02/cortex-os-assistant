...
1. stm_memory: A concise summary of the recent interactions and context of the user. \n
2. session_preferences:  A summary of the user's preferences and behavior during the current session, which can be used to personalize the AI's responses. \n
...
Context Summary must include the respective pattern, preferences and behavior summary of the user for that particular combination of mood and time of the day. \n

# Response: \n
Format response as JSON with the following structure:
```json
"emotional_profile": {{
    "emotional_level": "Integer in the range of 1-10",
    "logical_level": "Integer in the range of 1-10",
    "social_level": "Integer in the range of 1-10",
    "context_summary": "your_summary"
}}
...
SHOULD - Information that should be followed while generating responses, **Strong Preference** but can be violated in rare cases if it conflicts with other stronger preferences or important information (e.g. User prefers to have a morning workout routine) \n
...
CANNOT - Similar to Must, but in negative way. Include Information, which User does not want and should be strictly avoided while generating responses. **Strict Forbidden** and cannot be violated - (Example, "User CANNOT be asked about their personal information") \n
...
# Response:
Format response as List of Dictionaries in JSON with the following structure:
```json
[
    {{
        "category": "Select a category from following: [LIKE, DISLIKE, HABIT, FACT, STRICT_PREFERENCE]",
        "strictness": "Select from following: [MUST, SHOULD, CAN, CANNOT]",
        "content": "The corresponding content for that particular category and strictness level, which will be stored in the knowledge base and can be used for future reference and response generation"
    }},
]
```
