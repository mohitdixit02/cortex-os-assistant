...
class VoiceClient:
    ...
    async def _stream_audio_queue_response(self, taskItem: TaskItem) -> None:
        ...
        **Input**: \n
        ...
        if taskItem.result["response_type"] != "text_stream":
            logger.warning("Unsupported response type in task result: %s", taskItem.result["response_type"])
            return

        if self.audioBridge is None:
            logger.warning("Audio bridge is missing; cannot stream queue response")
            return
        ...
        async def _generator():
            ...
            async for token in iterate_tokens_async(
                ...
                if self.streamEvent and self.streamEvent.isUserSpeaking():
                    logger.info("Stopping queue token stream because user started speaking")
                    return
                ...
                if segment:
                    logger.info("Streaming audio for segment: %s", segment)
    ...
    async def stream_task_result(self, task_id: str) -> None:
        ...
        while True:
            if self.streamEvent and self.streamEvent.isCancelEventSet():
                logger.info("Stopping task result wait due to cancellation")
                return

            task_item = await MainTaskQueue.get_task_snapshot(task_id)
            if task_item is None:
                logger.warning("Task snapshot not found: %s", task_id)
                return
    ...
    async def _handle_task_queue(self, taskItem: TaskItem, cancel_event: asyncio.Event | None = None) -> None:
        ...
        """
        logger.info("task name: %s", taskItem.task_name)
        logger.info("task status: %s", taskItem.status)
        logger.info("task payload: %s", taskItem.result)
        try:
            if taskItem.status is TaskStatus.FAILED:
                logger.error("Task %s failed with error: %s", taskItem.task_id, taskItem.error)
                return
            ...
            logger.exception("Error handling task %s: %s", taskItem.task_id, e)
    ...
    async def process_tasks(self):
        ...
        """
        logger.info("VoiceClient.process_tasks is disabled in request-scoped mode.")
        while True:
